...
describe Puppet::Type.type(:elasticsearch_index_template) do
  let(:resource_name) { 'test_template' }

  include_examples 'REST API types', 'index_template', :content
    ...
    describe 'content and source validation' do
      ...
      it 'parses source paths into the content property' do
        ...
        allow(file_stub).to receive(:content)
          .and_return('{"index_patterns":["foobar-*"]}')
  ...
  describe 'insync?' do
    ...
    it 'is idempotent' do
      ...
      json = JSON.parse(File.read('spec/fixtures/templates/index.json'))
